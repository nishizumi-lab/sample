// カラー画像を色反転グレースケール画像に変換
function colorToRgray(color, rgray) {
  for (var i = 0; i < color.length; i += 4) {
    rgray[i/4] = 255 - (0.299*color[i+0] + 0.587*color[i+1] + 0.114*color[i+2]); 
  }
}
...
function showVideo(){
  ...
  var onFrame = function () {   
    ...
    colorToRgray(imgData.data, gray); 		            // カラー画像をグレースケール(色反転)に変換
    requestAnimationFrame(onFrame); 					// 再帰
    drawGrayImg(gray, ctx, imgData); 					// グレースケール画像を描画
