...
// 画像の描画
function imshow(ctx, imdata, im, type){
    // 入力がグレースケール画像の場合
    if(type == '0'){
        var n = im.length*4;
...
function showVideo(){
    ...
    var sharp = new Uint8ClampedArray(w*h*4);       // 出力画像格納用
    var btnflag = true;                             // ボタンの判定
    var kernel = [0,-1,0,                           // フィルタのカーネル
                -1,5,-1,
                0,-1,0];
    ...
    var onFrame = function () {
        ...
        filter2d(im.data, sharp, w, h, kernel, 1); // 画像にフィルタ処理
        requestAnimationFrame(onFrame);         // 再帰
        imshow(ctx, im, sharp, 1);              // 画像を描画   
    };
}
