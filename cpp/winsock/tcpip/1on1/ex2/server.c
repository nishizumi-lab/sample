...
int main(void){
	...
	listen(soc0, data_n); 					// TCPクライアントから接続があるまで待機
	while (1) {
		client_n = sizeof(client);
		soc = accept(soc0, (struct sockaddr *)&client, &client_n);	// TCPクライアントからの接続要求を受け付ける
		send(soc, data, data_n, 0); 		// ソケット通信でデータ「SOCKET」をクライアントに送信
		closesocket(soc); 					// TCPセッションの終了
	}
